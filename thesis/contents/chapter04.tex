...
但是對於麥克風系統卻產生非線性的響應輸出，使超聲波投影至人耳可聽見頻譜的聲音，而形成干擾噪聲
\cite{chen2019understanding}。
因此我們透過特別設計的超聲波麥克風干擾器，如系統實作\nameref{subsec:impl-mbox}，
並調整其輸出干擾聲音的音量，可獲得不同音量的干擾。
本實驗為模擬會談參與者之間的談話內容，使用台灣地區漢語語音噪音下聽辨測試(TMHINT)資料集\cite{wong2007development}，
...
實驗環境於背景噪音低於 40 dB的安靜室內空間進行。
    ...
    實驗步驟如下，首先於台灣地區漢語語音噪音下聽辨測試(TMHINT)資料集挑選出一標準文本。
接著使用Google 的文字轉換語音(TTS)服務朗讀標準文本，並夠過噪音計量測語音朗讀的音量，將朗讀的音量調整至 65 分貝。
接著使用系統實作\nameref{subsec:impl-mbox}的錄音克風，將朗讀標準文本錄音，取得參考原始聲音(未干擾的會談聲音紀律)。
接著透過系統實作\nameref{subsec:impl-mbox}的超音波麥克風干擾器與錄音克風，模擬會談進行時的干擾效果，
並調整干擾器的音量，取得於相同音量的標準文本語音朗讀下，數個不同干擾噪聲音量的疊加聲音紀錄。
...
隨著干擾器音量的遞增，在信噪比達到約 $-18~dB$ 之前，雖然明顯干擾噪聲遞增，但仍免強識別會談對話內容。
信噪比達到約 $-20~dB$ 時，干擾噪聲幾乎蓋過會談對話聲音，仍依稀聽有對話聲音，但會談對話內容已無法清楚識別。
當信噪比達到約 $-22~dB$ 時，僅聽得到干擾噪聲，無法聽識別出任何其他對話聲音。
