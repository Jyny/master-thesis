...
此時 \DEFunsealKey 為系統生命週期中\nameref{subsec:unseal}的關鍵變數。

    透過分析章節 \ref{sec:protocol} \nameref{sec:protocol}可以發現，
...
可能因此而失去機密性。
    ...
    透過分析章節 \ref{sec:protocol} \nameref{sec:protocol}
中的 \ref{subsec:protocol-init-reg} \nameref{subsec:protocol-init-reg}可以得知，
...
重送攻擊(Replay attack)等\cite{rfc5246}\cite{rfc8446}。
    ...
    綜上所述，本研究所設計之\nameref{sec:protocol}可以確保，在 \nameref{subsec:protocol-sessioning} 結束後，
直到成功解封 \nameref{subsec:protocol-unseal-access} 前，有效的會談聲音紀錄 \DEFrecREV 保有機密性。