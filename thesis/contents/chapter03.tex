...
\begin{table}[H]
    ...
    \begin{tabular}{ c c }
        ...
        \bf{符號} & \bf{釋義} \\
        \hline
        $Attender_{k}$ & 會議參與者 Attender 中，任意特定一位 \\
        $Owner_{i}$    & 會議參與者 Owner 中，任意特定一位 \\
        $SID$          & 當次會談唯一識別碼 \\
...
\begin{steps}
    ...
    \item Create Meeting Session：\\
        ...
        其中 $SID$ 為此次 {\it Meeting Session} 的唯一識別碼，
        $EK$ 為此次 {\it Meeting Session} 的 Session Key。
    ...
    \item Register Owner：\\
        ...
        使用此 URI 向 Server 進行系統機制「註冊會議擁有者」 (Register Owner)。運作方式如下：
        首先 $Owner_{i}$ 對 Server 發送 Register Owner 請求。Server 收到 Register Owner 請求後，
        隨即產生一組 ($PK_{i}$, $SK_{i}$)，並將請求來源 URI 中的 $SID$ 與 $PK_{i}$ 做關聯綁定，
        隨後回傳 $SK_{i}$ 給 $Owner_{i}$ ，且銷毀於 Server 中的$SK_{i}$，即完成此系統步驟。
        Onwer 為 Attender 中的特權角色，其特權來源為 $Owner_{i}$ 的 $SK_{i}$，
        因此 Owner 須妥善保管 $SK_{i}$，維持其機密性。在 {\it Single Owner Meeting Session} 情境中，
...
\begin{steps}
    ...
    \item End Owner Registration：\\
        MeetingBox 觸發 Start Session 後，首先對 Server 傳送請求 End Owner Registration。
