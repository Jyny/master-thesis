...
則定義 \DEFrecN $=$ $[n_{1}, ~n_{2}, ~ ...,~ n_{q}]$。

    理想上，響應輸出系統 \DEFfuncMicUSN{\DEFpause} 與 \DEFfuncMicUSJ{\DEFpause} 相同，
皆為音波麥克風干擾器於麥克風的響應輸出函數。因此理想上，我們應可以夠過
$[$ \DEFmicConv $~|~$ \DEFmicConv $=$ \DEFmicRecJ $-$ \DEFmicUSN $,~  p=1~...q]$，
獲得消除噪音後的有效會談聲音記錄 $[c_{1}, ~c_{2}, ~ ...,~ c_{q}]$。
...
將於次節 \ref{sec:estimate} \nameref{sec:estimate}中詳細說明。
    ...
    在輸入噪音聲音記錄與目標欲消除噪音兩者對齊後，若想提升噪音消除的效果，我們希望使輸入噪音聲音趨近於目標欲消除噪音。
因此建立一遞移函數，將轉換純噪音聲音響應輸出 \DEFmicUSN 趨近於目標欲消去噪音 \DEFmicRecJ，
定義此遞移函數為 \DEFfuncMicUSD{\DEFpause}。換句話說，我們希望得到一 \DEFfuncMicUSD{\DEFpause}，
使得 \DEFfuncUSDHT{\DEFpause} $\cdot$ \DEFfuncMicUSN{\DEFpause} 無限趨近於 \DEFfuncMicUSJ{\DEFpause}。
