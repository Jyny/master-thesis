...
\begin{steps}
    \item Create Meeting Session：
        ...
        會談參與者透過控制會談終端上的物理控制介面，使會談終端獲得外部事件，觸發 Create Meeting Session。

    \item Initialize Meeting Session：

            會談終端觸發 Create Meeting Session 後，
        首先對解封伺服器 \DEFserver 傳送請求 Initialize Meeting Session。
        會談終端透過解封伺服器的回覆獲取本次會談的元資料與加密金鑰。

    \item Upload Encrypted Noise：

            使用加密金鑰加密預先配置存在於會談終端 \DEFmeetingbox 的純噪音之聲音記錄，
        並上傳至解封伺服器 \DEFserver。

    \item Get Session URI：
...
\begin{steps}
    \item Start Session：
        ...
        使會談終端 \DEFmeetingbox 獲得外部事件，觸發 Start Session。
...
\begin{steps}
    \item Authorize Unsealing Service：
...
如圖 \ref{fig:m-o-init} 所示。
    ...
    其中 Step. 1\textasciitilde3，與 \ref{subsec:initialize} \nameref{fig:s-o-init} 相同。

    Step. 4\textasciitilde5，需要所有的會談主持者 \DEFownerAll 中的每一位會談主持者 \DEFowner 執行完成。
...
如圖 \ref{fig:m-o-sessioning} 所示。
    ...
    Step. 1\textasciitilde4 流程步驟與
\ref{subsec:sessioning} \nameref{fig:s-o-sessioning} 相同。
...
如圖 \ref{fig:m-o-unseal} 所示。
    ...
    Step. 1\textasciitilde4 流程步驟分別與 \ref{subsec:unseal} \nameref{fig:s-o-unseal} 相同。

    其中 Step. 1\textasciitilde2 需要所有的會談主持者 \DEFownerAll 執行，
各會談主持者 \DEFowner 可以同時獨立執行，待每位會談主持者 \DEFowner 執行成功後，即完成授權。
若有任何一位會談主持者 \DEFowner 執行失敗，即授權失敗。

    最後 Step. 3\textasciitilde4 為各會談主持者 \DEFowner 自行選擇執行與否，以取得此有效之會談聲音記錄。


\section{系統機制之一：聲音記錄存取控制協定}\label{sec:protocol}
...
\subsection{創建會談機制之協定}\label{subsec:protocol-init-create}
...
\subsection{註冊會談主持者機制之協定}\label{subsec:protocol-init-reg}

    於會談生命週期同一階段初始化會談中，當談終端 \DEFmeetingbox 獲得產自解封伺服器的元資料後，
會談終端上的人機互動介面提示予所有會談主持者 \DEFownerAll，可以進行「\nameref{fig:protocol-init-reg}」。
...
\begin{pmsgsi}
    ...
    待所有的會談主持者 \DEFownerAll 執行成功此機制後，即完成會談生命週期中的初始化會談。
\end{pmsgsi}


\subsection{進行會談機制之協定}\label{subsec:protocol-sessioning}

    在會談生命週期中進行會談階段(圖\ref{fig:m-o-sessioning})，
會談參與者透過與會談終端的物理控制介面(Step. 1、3)，
使會談終端獲得外部事件而觸發「\nameref{fig:protocol-sessioning}」。

    在此機制協定中，會談已完成會談生命週期第一階段初始化會談。
...
\subsection{會談主持者授權解封伺服器機制之協定}\label{subsec:protocol-unseal-auth}

    本章描述之協定為會談生命週期解封會談聲音記錄階段(圖\ref{fig:m-o-unseal})中的 Step. 1，
當會談主持者 \DEFowner 欲取得當時會談的聲音記錄，則執行此機制。
目標為使會談主持者 \DEFowner 授權解封伺服器 \DEFserver，使其有權限可以執行後續機制。

    在此機制協定中，會談主持者 \DEFowner 已完成前篇機制\nameref{subsec:protocol-init-reg}。
...
\subsection{取得會談聲音記錄機制之協定}
    ...
    本章描述之協定為會談生命週期解封會談聲音記錄階段(圖\ref{fig:m-o-unseal})中的 Step. 2，
當解封伺服器 \DEFserver 獲得所有會談主持者 \DEFownerAll 的授權後，欲取得當時會談的聲音記錄，則執行此機制。
...
所有會談主持者的唯一識別碼 \DEFownerID、與屬於各會談主持者的授權金鑰 \DEFagentKey。
    ...
    機此機制協定目標為，透過談主持者 \DEFowner 於解封伺服器 \DEFserver 的授權金鑰 \DEFagentKey，
...
\section{系統機制之二：自適應噪音消除}\label{sec:anc}

    本節將說明於生命週期解封會談聲音記錄中，如何應用自適應噪音消除，以還原出有效的會談聲音記錄。
...
\section{系統機制之三：聲音樣本的離散時間誤差推估}\label{sec:estimate}
...
\section{系統理論與假設}\label{sec:theory}
