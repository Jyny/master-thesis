...
\end{center}
    ...
    換句話說，希望得到一遞移函數 \DEFfuncAf{\DEFpause}，透過轉換純超聲波麥克風干擾器的響應輸出
...
如圖 \ref{fig:estimate-sub} 所示。在已對齊的受干擾的聲音(上)與純噪音(中)，於時域上的波形表現高度相似。
且在整個離散時間序列每個樣本相減的結果(兩聲音干涉抵銷)，可見能量大幅度衰減(下)。

    因此本研究提出，透過累計整個離散時間序列每個樣本相減(兩聲音干涉抵銷)後總能量差的大小，作為是否對齊的參考指標。
當找到一動態相對平移的量 \DEFcandiSFT ，其干涉抵銷後總能量差為最小值，此時為對齊點。
即此時的動態相對平移的量 \DEFcandiSFT，即為聲音樣本的離散時間誤差值，定義為 \DEFshift。
...
最終得到特定動態相對平移的量 \DEFcandiSFT 之下的累計能量差。
...
我們可以得知在在特定動態相對平移的量 \DEFcandiSFT 之下的累計能量差，
理想上透過歷遍所有可能的 \DEFcandiSFT，即可以找出最小的累計能量差，因而找到聲音樣本的離散時間誤差值 \DEFshift。
但若分析歷遍所有可能 \DEFcandiSFT 的累計能量差的時間複雜度，會發現歷遍所有可能是沒有效率的。
推導如下。定義 $n$ 的值為離散時間序列長度 \DEFtimeLen。
...
因此歷遍所有可能 \DEFcandiSFT 的累計能量差其時間複雜度，每增加一秒，
