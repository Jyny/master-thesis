...
\pseudocode[
    head=Protocol for Initialize Meeting Session,
    mode=text,
    codesize=\scriptsize,
]{
    \\
    \bf{\normalsize Meetingbox} \> \> \bf{\normalsize Server} \\
    $\{$ \DEFrecN $\}$ \> \> \\
    [0.5\baselineskip][\hline] \\
    %
    $M_1$ $\leftarrow$ $\{$\DEFrecN$\}$ \> \> \\
    %
    \> \sendmessageright *{\text{$M_1$}} \> \\
    %
    \> \> \DEFunsealKey $\leftarrow$ $KGen()$ \\
    \> \> \DEFsessionID $\leftarrow$ $IDGen()$ \\
    ...
    %
    \> \> {\bf BIND RELATIONS:}\\
    \> \> \pcind $m_3$ ~ {\bf BELONGS TO} ~ \DEFsessionID
}
\end{pcvstack}
