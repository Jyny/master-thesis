...
於會談結束後，受超聲波麥克風干擾器干擾的會談聲音記錄 ($REC_{J}$) 將透過本研究所設計的「解封」 ({\it Unseal}) 機制，
使會談參與者 Attender 中的會談擁有者 Owner，得以獲得的系統機制所產生的有效聲音記錄 ($REC_{rev}$)。
...
有能力決定是否「解封」 ({\it Unseal}) 會談聲音記錄，並獲取系統機制所產生的有效聲音記錄 ($REC_{rev}$)。
...
其角色為，執行系統機制「解封」 ({\it Unseal})。此機制觸發於會談結束後，Owner 向 Server 發送請求執行。
