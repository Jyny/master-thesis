...
\section{系統機制}
    ...
    本章節將詳細說明各項系統機制，
包含基於前章節 \ref{sec:system-flow} \nameref{sec:system-flow} 所提出的聲音記錄存取控制協定。
以及於生命週期解封會談錄音中，如何應用自適應噪音消除，以還原出有效的會談錄音。
最後說明如何推估聲音樣本的離散時間誤差值，用於自適應噪音消除時對齊聲音樣本。

\subsection{聲音記錄存取控制協定}

    本節基於前章節 \ref{sec:system-flow} \nameref{sec:system-flow}，發展出一套聲音記錄存取控制協定。
...
\begin{tabularx}{0.95\textwidth} {
    ...
    \hline
\end{tabularx}
\captionof{figure}{初始化會談之一：創建會談之協定}\label{fig:protocol-init-create}
...
\begin{pmsgs}
    ...
    觸發會談終端向解封伺服器 \DEFserver 發送此請求，請求訊息內容為空。
    ...
    接著將 \DEFunsealKey 關聯綁定屬於 \DEFsessionID 後，回傳 \DEFsessionID 與 \DEFunsealKey。
    ...
    隨後回覆會談終端 \DEFmeetingbox 上傳成功，訊息內容為空。
...
\begin{tabularx}{0.95\textwidth} {
    ...
    \hline
\end{tabularx}
\captionof{figure}{初始化會談之二：註冊會談主持者之協定}\label{fig:protocol-init-reg}
\setstretch{1.2}\normalsize\end{center}

\begin{pmsgs}
    \item \DEFowner $\rightarrow$ \DEFmeetingbox：

    \item \DEFmeetingbox $\rightarrow$ \DEFowner：
...
\begin{tabularx}{0.95\textwidth} {
    ...
    \hline
\end{tabularx}
\captionof{figure}{解封會談聲音記錄之一：會談主持者授權解封伺服器之協定}\label{fig:protocol-unseal-auth}
\setstretch{1.2}\normalsize\end{center}
...
\begin{tabularx}{0.95\textwidth} {
    ...
    \hline
\end{tabularx}
\captionof{figure}{解封會談聲音記錄之二：取得會談聲音記錄之協定}\label{fig:protocol-unseal-access}
\setstretch{1.2}\normalsize\end{center}
